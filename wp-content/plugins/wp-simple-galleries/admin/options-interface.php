 ...
 */
function wpsg_optionsframework_fields() {
    ...
    foreach ($options as $value) {
        ...
        // Wrap all options
        if (($value['type'] != "heading") && ($value['type'] != "info") && ($value['type'] != "donate")) {
        ...
        switch ($value['type']) {
            ...
            case 'donate':
                $output .= '<div class="section"><h4 class="heading">' . $value['name'] . '</h4>
                    <p>' . $value['desc'] . '</p>
            ...
            case "multicheck":
                foreach ($value['options'] as $key => $option) {
                    ...
                    $output .= '<input id="' . esc_attr($id) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr($name) . '" ' . $checked . ' /><label for="' . esc_attr($id) . '">' . ucwords(esc_html($label)) . '</label>';
                }
                break;
            ...
            // Uploader
            case "upload":
                $output .= wpsg_optionsframework_medialibrary_uploader($value['id'], $val, null); // New AJAX Uploader using Media Library	
                break;
            ...
            case 'typography':
                ...
                $faces = wpsg_of_recognized_font_faces();
                ...
                /* Font Style */
                $styles = wpsg_of_recognized_font_styles();
            ...
            case 'background':
                ...
                $output .= wpsg_optionsframework_medialibrary_uploader($value['id'], $background['image'], null, '', 0, 'image');
                ...
                $repeats = wpsg_of_recognized_background_repeat();
                ...
                $positions = wpsg_of_recognized_background_position();
                ...
                $attachments = wpsg_of_recognized_background_attachment();
            ...
            case "info":
                ...
                if ($value['desc']) {
                    $output .= apply_filters('wpsg_of_sanitize_info', $value['desc']) . "\n";
                }
